...
const seed = ({ topicData, userData, articleData, commentData }) => {
  return db.query('DROP TABLE IF EXISTS comments;').then(()=>{
    ...
    const formattedTopics = topics.map((topic)=>{return [topic.slug,topic.description,topic.img_url]})
    const insertTopicsQuery = format('INSERT INTO topics(slug,description,img_url) VALUES %L',formattedTopics)
    return db.query(insertTopicsQuery)
  }).then(()=>{
    const formattedUsers = users.map((user)=>{return [user.username,user.name,user.avatar_url]})
